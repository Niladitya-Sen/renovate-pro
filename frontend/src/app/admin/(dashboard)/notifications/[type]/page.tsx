...
function NotificationsCard({ title, date }: Readonly<{ title: string, date: string }>) {
    ...
    return (
        <div className='bg-white p-4 flex gap-4 items-center border rounded-sm hover:shadow-md transition-shadow duration-300'>
            ...
            <button className="border border-blue-500 text-blue-500 hover:bg-secondary transition-colors duration-200 px-4 py-2 rounded-sm">View</button>
        </div>
    )
}
        ...
        <>
            <div className='mb-4 flex flex-wrap gap-4 items-center justify-between'>
                <div className='space-x-4'>
                    <Link href="/admin/notifications/all" className={buttonVariants({
                        variant: type === 'all' ? 'default' : 'secondary'
                    })}>All</Link>
                    <Link href="/admin/notifications/unread" className={buttonVariants({
